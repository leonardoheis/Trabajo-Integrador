...
| `indexing/` | Entry point: one accepted document → persisted chunks. Resolves municipal metadata from the scrapper CSVs. | `IndexerService`, `IndexResult`, `CsvDocumentMetadataRepository` |
...
| `llm/` | Streaming chat completion, one module per provider. | `ChatLlm` (abstract), `ClaudeChatLlm`, `LlamaCppChatLlm` |
...
`anthropic` *and* `llama_cpp` the moment anything imported `knowledge.utils.text`. That
cost lands on the test suite, which needs none of them. So:
...
(`ChromaVectorStore` ↔ `InMemoryVectorStore`; `ClaudeChatLlm` ↔ `LlamaCppChatLlm` via
`Settings.chat_llm_provider`), so consumers need a common type to name. Annotating
against one concrete class would be false, and a union would force `chat/` to import
both providers.

Both ABC modules are kept free of heavy imports on purpose: they sit on the import path
of `retrieval/`, `indexing/` and `chat/`.

`embeddings/` and `indexing/`'s metadata source have a single production implementation
each and therefore no abstraction — consumers name
`SentenceTransformerEmbedder` and `CsvDocumentMetadataRepository` directly.
...
| `llm/` | `ChatLlmError`, `ChatRefusalError` |
| `indexing/` | `MetadataSourceError` |
