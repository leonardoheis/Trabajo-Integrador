...
from tests.knowledge.fakes import FAKE_ENTITIES, TEXT
...
class TestIndexerService:
    async def test_indexes_chunks_with_metadata(
        ...
        result = await indexer.index(
            "job-1", "ordenanza_10902_2026.pdf", "sha-1", TEXT, FAKE_ENTITIES
        )
    ...
    async def test_reindexing_the_same_document_is_idempotent(
        ...
        await indexer.index("job-2", "ordenanza_10902_2026.pdf", "sha-1", TEXT, FAKE_ENTITIES)
    ...
    async def test_empty_text_indexes_nothing_but_still_resolves_metadata(
        ...
        result = await indexer.index(
            "job-1", "ordenanza_10902_2026.pdf", "sha-1", "   ", FAKE_ENTITIES
        )
