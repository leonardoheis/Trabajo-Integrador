...
class TestChunkerService:
    ...
    def test_every_chunk_carries_the_context_header(self) -> None:
        ...
        assert len(chunks) > 1
        for chunk in chunks:
            assert chunk.text.startswith("Decreto 810/2026")

    def test_header_falls_back_to_filename_without_resolved_entities(self) -> None:
    ...
    def test_store_metadata_is_flat(self) -> None:
        chunker = ChunkerService(chunk_size=500, chunk_overlap=50)

        chunk = chunker.split("Texto.", "job-1", "abc", _META)[0]
